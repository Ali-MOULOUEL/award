...
def bag_to_csv(options, fname):
    directoryName = (fname[fname.rfind('/'): -4])
    newDirectoryName = directoryName[1:]
...
def GetTopicList(path):
    bag = rosbag.Bag(path)
    topics = {"/scan_vlp32_front", "/askClear/feedback","/debug/anticollision_limiters", "/command", "/gps_position", "/gps_bestpos", "/vehicle_pose", "/vehicle_velocity_meas", "/vehicle_state_common"}
    print(f"{topics=}")
    types = []
